...
export async function load({
  ...
  let blazeface: tfconv.GraphModel;
  const localStorageHandler = getApp().globalData.localStorageIO('blazeface_model');
  try {
    console.log("load model from local storage")
    blazeface = await tfconv.loadGraphModel(localStorageHandler);
  } catch (e) {
    console.log(e)
    if(modelUrl != null){
      ...
      console.log(`load model from ${BLAZEFACE_MODEL_URL}`)
